...
def verify_ipa(package):
    with zipfile.ZipFile(package) as archive:
        names = archive.namelist()
    if 'Payload/Runner.app/Runner' not in names:
        raise SystemExit('未签名 IPA 缺少 Payload/Runner.app/Runner：' + str(package))
    signed = [name for name in names if name.startswith('Payload/Runner.app/_CodeSignature/')
              or name == 'Payload/Runner.app/embedded.mobileprovision']
    if signed:
        raise SystemExit('未签名 IPA 含有签名内容：' + ', '.join(sorted(signed)[:3]))
...
def main():
    ...
    if options.export_options:
        ...
        payload_root = root / 'build' / 'ios' / 'ipa-payload'
        if payload_root.exists():
            shutil.rmtree(payload_root)
        payload = payload_root / 'Payload'
        payload.mkdir(parents=True)
        run(['ditto', str(application), str(payload / 'Runner.app')])
        package = output / f'{variant.slug}-{version}-ios-unsigned.ipa'
        run(['ditto', '-c', '-k', '--sequesterRsrc', '--keepParent', str(payload), str(package)])
        verify_ipa(package)
